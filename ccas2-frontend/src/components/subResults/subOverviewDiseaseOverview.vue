  ...
  <v-sheet class="mx-3">
    ...
    <v-row align="center" class="pt-6">
      ...
      <v-col cols="2">
        ...
        <v-sheet :class="itemsContentClass"> {{
            dat.jobinfo.email == null ? "Not provided" : dat.jobinfo.email
          }}
        </v-sheet>
      </v-col>
      <v-col cols="6">
        ...
        <v-sheet :class="itemsClass">Submit Data Types
          <common-help-message>
            ...
            There are 4 categories of omics data are supported in CCAS incuding SNV/Indels, Copy number variation,
            Expression and Methylation.<br/>
    ...
    <v-row align="center" class="pt-6">
      ...
      <v-col cols="2">
        ...
        <v-sheet :class="itemsContentClass">
          <a :href="$commonfunc.renderLinkDOID(dat.data.doid)" class="teal--text" style="text-decoration: none"
             target="_blank">{{ dat.data.doid }}</a>
        </v-sheet>
      </v-col>
      <v-col cols="2">
        <v-sheet :class="itemsClass">MeSH ID:</v-sheet>
        <v-sheet :class="itemsContentClass">
          <a :href="$commonfunc.renderLinkMESHID(dat.data.MESHID.replace('MESH:',''))" class="teal--text"
             style="text-decoration: none"
             target="_blank">{{ dat.data.MESHID.replace('MESH:', '') }} &nbsp;&nbsp;</a>
        </v-sheet>
      </v-col>
      <v-col cols="6">
        <v-sheet :class="itemsClass">Parent Disease Ontology ID:</v-sheet>
        <v-sheet :class="itemsContentClass">
          <a v-for="x in dat.data.is_a" :key="x" :href="$commonfunc.renderLinkDOID(x)" class="teal--text"
             style="text-decoration: none" target="_blank">{{ x }} &nbsp;&nbsp;</a>
